...
class Certificate(models.Model):
    ...
    def _nf_zatca_get_issuer_name(self):
        self.ensure_one()
    ...
    @api.model
    def _nf_zatca_get_csr_str(self, journal):
        """
        ...
        x509_alt_names_extension = x509.SubjectAlternativeName([
            x509.DirectoryName(x509.Name([
                ...
                x509.NameAttribute(ObjectIdentifier('2.5.4.4'), '1-Odoo|2-%s|3-%s' % (
                    version_info['server_version_info'][0], journal.nf_zatca_serial_number)),
                ...
                x509.NameAttribute(NameOID.TITLE, company_id._nf_zatca_get_csr_invoice_type()),
                # Location
        ...
        x509_extensions = (
            ...
            (x509.UnrecognizedExtension(ObjectIdentifier('1.3.6.1.4.1.311.20.2'),
                                        CERT_TEMPLATE_NAME[company_id.nf_zatca_api_mode]), False),
        ...
        private_key = serialization.load_pem_private_key(base64.b64decode(company_id.nf_zatca_private_key_id.pem_key), password=None)
