...
class AccountMove(models.Model):
    ...
    nf_zatca_uuid = fields.Char(string='Document UUID (SA)', copy=False, help="Universally unique identifier of the Invoice")

    nf_zatca_invoice_signature = fields.Char("Unsigned XML Signature", copy=False)

    nf_zatca_chain_index = fields.Integer(
    ...
    def _nf_zatca_is_simplified(self):
        """
    ...
    @api.depends('amount_total_signed', 'amount_tax_signed', 'l10n_sa_confirmation_datetime', 'company_id',
                 'company_id.vat', 'journal_id', 'journal_id.nf_zatca_production_csid_json', 'edi_document_ids',
                 'nf_zatca_invoice_signature', 'nf_zatca_chain_index', 'state')
    def _compute_qr_code_str(self):
        ...
        for move in self:
            ...
            if move.country_code == 'SA' and move.move_type in ('out_invoice', 'out_refund') and zatca_document and move.state != 'draft':
                qr_code_str = ''
                if move._nf_zatca_is_simplified():
                    x509_cert_sudo = move.journal_id.sudo().nf_zatca_production_csid_certificate_id
                    xml_content = self.env.ref('nf_zatca_integration.zatca_account_edi_format')._nf_zatca_generate_zatca_template(move)
                    qr_code_str = move._nf_zatca_get_qr_code(move.journal_id, xml_content, x509_cert_sudo,
                                                            move.nf_zatca_invoice_signature, True)
    ...
    def _nf_zatca_get_qr_code_encoding(self, tag, field, int_length=1):
        """
    ...
    def _nf_zatca_check_refund_reason(self):
        """
    ...
    @api.model
    def _nf_zatca_get_qr_code(self, journal_id, unsigned_xml, certificate, signature, is_b2c=False):
        """
        ...
        def xpath_ns(expr):
            return root.xpath(expr, namespaces=edi_format._nf_zatca_get_namespaces())[0].text.strip()
        ...
        if invoice_datetime and journal_id.company_id.vat and certificate and signature:
            ...
            invoice_hash = edi_format._nf_zatca_generate_invoice_xml_hash(prehash_content, 'digest')

            amount_total = float(xpath_ns('//cbc:TaxInclusiveAmount'))
            amount_tax = float(xpath_ns('//cac:TaxTotal/cbc:TaxAmount'))
            seller_name_enc = self._nf_zatca_get_qr_code_encoding(1, journal_id.company_id.display_name.encode())
            seller_vat_enc = self._nf_zatca_get_qr_code_encoding(2, journal_id.company_id.vat.encode())
            timestamp_enc = self._nf_zatca_get_qr_code_encoding(3,
                                                               invoice_datetime.strftime("%Y-%m-%dT%H:%M:%S").encode())
            amount_total_enc = self._nf_zatca_get_qr_code_encoding(4, float_repr(abs(amount_total), 2).encode())
            amount_tax_enc = self._nf_zatca_get_qr_code_encoding(5, float_repr(abs(amount_tax), 2).encode())
            invoice_hash_enc = self._nf_zatca_get_qr_code_encoding(6, invoice_hash)
            signature_enc = self._nf_zatca_get_qr_code_encoding(7, signature.encode())
            public_key_enc = self._nf_zatca_get_qr_code_encoding(8, base64.b64decode(certificate._get_public_key_bytes(formatting='base64')))
            ...
            if is_b2c:
                qr_code_str += self._nf_zatca_get_qr_code_encoding(9, base64.b64decode(certificate._get_signature_bytes(formatting='base64')))
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        for move in self:
            ...
            if move.nf_zatca_chain_index:
                move.show_reset_to_draft_button = False

    def _nf_zatca_reset_confirmation_datetime(self):
    ...
    def _nf_zatca_generate_unsigned_data(self):
        """
        ...
        """
        self.ensure_one()
        edi_format = self.env.ref('nf_zatca_integration.zatca_account_edi_format')
        ...
        self.nf_zatca_uuid = uuid.uuid4()
        # We generate the XML content
        xml_content = edi_format._nf_zatca_generate_zatca_template(self)
        # Once the required values are generated, we hash the invoice, then use it to generate a Signature
        invoice_hash_hex = self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_generate_invoice_xml_hash(xml_content).decode()
        self.nf_zatca_invoice_signature = edi_format._nf_zatca_get_digital_signature(self.journal_id.company_id,
                                                                                   invoice_hash_hex).decode()
        return xml_content

    def _nf_zatca_log_results(self, xml_content, response_data=None, error=False):
        """
            Save submitted invoice XML hash in case of either Rejection or Acceptance.
        """
        self.ensure_one()
        self.journal_id.nf_zatca_latest_submission_hash = self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_generate_invoice_xml_hash(
            xml_content)
    ...
    def _is_nf_zatca_eligibile_invoice(self):
        self.ensure_one()
    ...
    def _get_report_base_filename(self):
        ...
        """
        if self._is_nf_zatca_eligibile_invoice():
            return self.with_context(nf_zatca_file_format=False).env['account.edi.xml.ubl_21.zatca']._export_invoice_filename(self)
        return super()._get_report_base_filename()

    def _get_invoice_report_filename(self, extension='pdf'):
        if self._is_nf_zatca_eligibile_invoice():
            return self.with_context(nf_zatca_file_format=extension).env['account.edi.xml.ubl_21.zatca']._export_invoice_filename(self)
        return super()._get_invoice_report_filename(extension)

    def _nf_zatca_is_in_chain(self):
        """
    ...
    def action_post(self):
        ...
        # Automatically submit the invoice to ZATCA if it is a Saudi Invoice
        for move in self.filtered(lambda move: move.country_code == "SA" and move.move_type in ('out_invoice', 'out_refund')):
...
class AccountMoveLine(models.Model):
    ...
    def _apply_retention_tax_filter(self, tax_values):
        return not tax_values['tax_id'].nf_zatca_is_retention
        ...
        for line in self:
            if (
                ...
                line.l10n_gcc_invoice_tax_amount = sum(
                    ...
                    if not tax_data['tax'].nf_zatca_is_retention
                )
