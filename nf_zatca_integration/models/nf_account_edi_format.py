...
class AccountEdiFormat(models.Model):
    ...
    def _nf_zatca_get_zatca_datetime(self, timestamp):
        return fields.Datetime.context_timestamp(self.with_context(tz='Asia/Riyadh'), timestamp)

    def _nf_zatca_xml_node_content(self, root, xpath, namespaces=None):
        namespaces = namespaces or self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_get_namespaces()
        return etree.tostring(root.xpath(xpath, namespaces=namespaces)[0], with_tail=False,
                              encoding='utf-8', method='xml')

    @api.model
    def _nf_zatca_get_digital_signature(self, company_id, invoice_hash):
        """
            Generate an ECDSA SHA256 digital signature for the XML eInvoice
        """
        decoded_hash = b64decode(invoice_hash).decode()
        return company_id.sudo().nf_zatca_private_key_id._sign(decoded_hash, formatting='base64')

    def _nf_zatca_calculate_signed_properties_hash(self, issuer_name, serial_number, signing_time, public_key):
        """
        ...
        """
        signed_properties = etree.fromstring(self.env['ir.qweb']._render('nf_zatca_integration.export_sa_zatca_ubl_signed_properties', {
    ...
    def _nf_zatca_sign_xml(self, xml_content, certificate, signature):
        """
        ...
        issuer_name = certificate._nf_zatca_get_issuer_name()
        serial_number = certificate.serial_number
        signing_time = self._nf_zatca_get_zatca_datetime(datetime.now()).strftime('%Y-%m-%dT%H:%M:%SZ')
        public_key_hashing = b64encode(sha256(der_cert).hexdigest().encode()).decode()

        signed_properties_hash = self._nf_zatca_calculate_signed_properties_hash(issuer_name, serial_number,
        ...
        invoice_hash = self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_generate_invoice_xml_hash(prehash_content,
                                                                                                   'digest')
    ...
    def _nf_zatca_assert_clearance_status(self, invoice, clearance_data):
        """
            Assert Clearance status. To be overridden in case there are any other cases to be accounted for
        """
        mode = 'reporting' if invoice._nf_zatca_is_simplified() else 'clearance'
    ...
    def _nf_zatca_postprocess_zatca_template(self, xml_content):
        """
    ...
    def _nf_zatca_generate_zatca_template(self, invoice):
        """
        ...
        return self._nf_zatca_postprocess_zatca_template(xml_content)

    def _nf_zatca_submit_einvoice(self, invoice, signed_xml, PCSID_data):
        """
        ...
        """
        clearance_data = invoice.journal_id._nf_zatca_api_clearance(invoice, signed_xml.decode(), PCSID_data)
        ...
        if not clearance_data.get('error'):
            return self._nf_zatca_assert_clearance_status(invoice, clearance_data)
        return clearance_data

    def _nf_zatca_postprocess_einvoice_submission(self, invoice, signed_xml, clearance_data):
        """
        ...
        """
        if invoice._nf_zatca_is_simplified():
    ...
    def _nf_zatca_apply_qr_code(self, invoice, xml_content):
        """
    ...
    def _nf_zatca_get_signed_xml(self, invoice, unsigned_xml, certificate):
        """
        ...
        """
        signed_xml = self._nf_zatca_sign_xml(unsigned_xml, certificate, invoice.nf_zatca_invoice_signature)
        if invoice._nf_zatca_is_simplified():
            ...
            return self._nf_zatca_apply_qr_code(invoice, signed_xml)
        return signed_xml

    def _nf_zatca_export_zatca_invoice(self, invoice, xml_content=None):
        """
        ...
        unsigned_xml = xml_content or self._nf_zatca_generate_zatca_template(invoice)

        # Load PCISD data and certificate
        try:
            PCSID_data, certificate = invoice.journal_id._nf_zatca_api_get_pcsid()
        ...
        try:
            signed_xml = self._nf_zatca_get_signed_xml(invoice, unsigned_xml, certificate_sudo)
        except UserError as e:
            return ({
        ...
        return self._nf_zatca_submit_einvoice(invoice, signed_xml, PCSID_data), signed_xml

    def _nf_zatca_check_partner_missing_info(self, partner_id, fields_to_check):
        """
    ...
    def _nf_zatca_check_seller_missing_info(self, invoice):
        """
        ...
        fields_to_check = [
            ('nf_zatca_building_number', _('Building Number for the Buyer is required on Standard Invoices')),
            ...
            ('vat',
             ...
             lambda p, v: p.nf_zatca_additional_identification_scheme != 'TIN'
             ),
            ('state_id', _('State / Country subdivision'))
        ]
        return self._nf_zatca_check_partner_missing_info(partner_id, fields_to_check)

    def _nf_zatca_check_buyer_missing_info(self, invoice):
        """
            Helper function to check if ZATCA mandated partner fields are missing for the buyer
        """
        fields_to_check = []
        if any(tax.nf_zatca_exemption_reason_code in ('VATEX-SA-HEA', 'VATEX-SA-EDU') for tax in
        ...
        elif invoice.commercial_partner_id.nf_zatca_additional_identification_scheme == 'TIN':
            fields_to_check += [
                ('vat', _('VAT is required when Identification Scheme is set to Tax Identification Number'))
            ]
        if not invoice._nf_zatca_is_simplified() and invoice.partner_id.country_id.code == 'SA':
        ...
        return self._nf_zatca_check_partner_missing_info(invoice.commercial_partner_id, fields_to_check)

    def _nf_zatca_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        """
        ...
        chain_head = invoice.journal_id._nf_zatca_get_last_posted_invoice()
        if chain_head and chain_head != invoice and not chain_head._nf_zatca_is_in_chain():
            return {invoice: {
        ...
        xml_content = None
        if not invoice.nf_zatca_chain_index:
            ...
            invoice.nf_zatca_chain_index = invoice.journal_id._nf_zatca_get_next_chain_index()
            xml_content = invoice._nf_zatca_generate_unsigned_data()
        ...
        response_data, submitted_xml = self._nf_zatca_export_zatca_invoice(invoice, xml_content)
        ...
        if response_data.get('error'):
            ...
            if response_data.get('rejected'):
                invoice._nf_zatca_log_results(submitted_xml, response_data, error=True)
            ...
            if not response_data.get('excepted'):
                invoice.nf_zatca_chain_index = False
        ...
        cleared_xml = self._nf_zatca_postprocess_einvoice_submission(invoice, submitted_xml, response_data)

        # Save the submitted/returned invoice XML content once the submission has been completed successfully
        invoice._nf_zatca_log_results(cleared_xml.encode(), response_data)
        return {
            invoice: {
                'success': True,
    ...
    def _check_move_configuration(self, invoice):
        ...
        if not company.sudo().nf_zatca_private_key_id:
            errors.append(
        ...
        if not journal.nf_zatca_serial_number:
            errors.append(
        ...
        supplier_missing_info = self._nf_zatca_check_seller_missing_info(invoice)
        customer_missing_info = self._nf_zatca_check_buyer_missing_info(invoice)
        ...
        if invoice.move_type in ('in_refund', 'out_refund') and not invoice._nf_zatca_check_refund_reason():
            errors.append(
    ...
    def _nf_zatca_get_invoice_content_edi(self, invoice):
        """
        ...
        return doc.attachment_id.raw or self._nf_zatca_generate_zatca_template(invoice).encode()
    ...
    def _get_move_applicability(self, move):
        ...
        return {
            'post': self._nf_zatca_post_zatca_edi,
            'edi_content': self._nf_zatca_get_invoice_content_edi,
        }
