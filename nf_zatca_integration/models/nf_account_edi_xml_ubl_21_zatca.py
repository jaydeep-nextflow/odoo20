...
class AccountEdiXmlUBL21Zatca(models.AbstractModel):
    ...
    def _nf_zatca_get_namespaces(self):
        """
    ...
    def _nf_zatca_generate_invoice_xml_sha(self, xml_content):
        """
        ...
        def _canonicalize_xml(content):
            ...
            return etree.tostring(content, method="c14n", exclusive=False, with_comments=False,
                                  inclusive_ns_prefixes=self._nf_zatca_get_namespaces())
        ...
        def _transform_and_canonicalize_xml(content):
            ...
            invoice_xsl = etree.parse(file_path('nf_zatca_integration/data/pre-hash_invoice.xsl'))
    ...
    def _nf_zatca_generate_invoice_xml_hash(self, xml_content, mode='hexdigest'):
        """
        ...
        """
        xml_sha = self._nf_zatca_generate_invoice_xml_sha(xml_content)
        if mode == 'hexdigest':
    ...
    def _nf_zatca_get_previous_invoice_hash(self, invoice):
        """ Function that returns the Base 64 encoded SHA256 hash of the previously submitted invoice """
        if invoice.company_id.nf_zatca_api_mode == 'sandbox' or not invoice.journal_id.nf_zatca_latest_submission_hash:
            # If no invoice, or if using Sandbox, return the b64 encoded SHA256 value of the '0' character
            return "NWZlY2ViNjZmZmM4NmYzOGQ5NTI3ODZjNmQ2OTZjNzljMmRiYzIzOWRkNGU5MWI0NjcyOWQ3M2EyN2ZiNTdlOQ=="
        return invoice.journal_id.nf_zatca_latest_submission_hash
    ...
    def _get_partner_party_identification_vals_list(self, partner):
        """ Override to include/update values specific to ZATCA's UBL 2.1 specs """
        return [{
            'id_attrs': {'schemeID': partner.nf_zatca_additional_identification_scheme},
            'id': (
                partner.nf_zatca_additional_identification_number
                if partner.nf_zatca_additional_identification_scheme != 'TIN' and partner.country_code == 'SA'
                else partner.vat
            ),
        }]
    ...
    def _nf_zatca_get_payment_means_code(self, invoice):
    ...
    def _get_invoice_payment_means_vals_list(self, invoice):
        ...
        res[0]['payment_means_code'] = PAYMENT_MEANS_CODE.get(self._nf_zatca_get_payment_means_code(invoice), PAYMENT_MEANS_CODE['unknown'])
    ...
    def _get_partner_address_vals(self, partner):
        ...
        return {
            **super()._get_partner_address_vals(partner),
            'building_number': partner.nf_zatca_building_number,
            'city_subdivision_name ': partner.street2,
            'plot_identification': partner.nf_zatca_plot_identification,
        }
    ...
    def _export_invoice_filename(self, invoice):
        ...
        file_format = self.env.context.get('nf_zatca_file_format', 'xml')
        if file_format:
            file_name = f'{file_name}.{file_format}'
        return file_name

    def _nf_zatca_get_invoice_transaction_code(self, invoice):
        """
        ...
        """
        return '0%s00%s00' % (
            '2' if invoice._nf_zatca_is_simplified() else '1',
            '1' if invoice.commercial_partner_id.country_id != invoice.company_id.country_id and not invoice._nf_zatca_is_simplified() else '0'
        )

    def _nf_zatca_get_invoice_type(self, invoice):
        """
    ...
    def _nf_zatca_get_billing_reference_vals(self, invoice):
        """ Get the billing reference vals required to render the BillingReference for credit/debit notes """
        if self._nf_zatca_get_invoice_type(invoice) != 388:
            return {
    ...
    def _apply_invoice_tax_filter(self, base_line, tax_values):
        ...
        res = not tax_id.nf_zatca_is_retention
        ...
        if not base_line['record'].move_id._is_downpayment():
            return not tax_id.nf_zatca_is_retention and not base_line['record']._get_downpayment_lines()
        return res
    ...
    def _nf_zatca_get_prepaid_amount(self, invoice, vals):
        ...
        if downpayment_lines:
            tax_vals = invoice._prepare_invoice_aggregated_taxes(
                filter_tax_values_to_apply=lambda l, t: not self.env['account.tax'].browse(t.get('id')).nf_zatca_is_retention
            )
    ...
    def _nf_zatca_get_monetary_vals(self, invoice, vals):
        ...
        downpayment_vals = self._nf_zatca_get_prepaid_amount(invoice, vals)
    ...
    def _get_tax_category_list(self, customer, supplier, taxes):
        ...
        non_retention_taxes = taxes.filtered(lambda t: not t.nf_zatca_is_retention)
    ...
    def _export_invoice_vals(self, invoice):
        ...
        vals.update({
            ...
            'InvoiceType_template': 'nf_zatca_integration.ubl_21_InvoiceType_zatca',
            'CreditNoteType_template': 'nf_zatca_integration.ubl_21_CreditNoteType_zatca',
            'DebitNoteType_template': 'nf_zatca_integration.ubl_21_DebitNoteType_zatca',
            'InvoiceLineType_template': 'nf_zatca_integration.ubl_21_InvoiceLineType_zatca',
            'CreditNoteLineType_template': 'nf_zatca_integration.ubl_21_CreditNoteLineType_zatca',
            'DebitNoteLineType_template': 'nf_zatca_integration.ubl_21_DebitNoteLineType_zatca',
            'AddressType_template': 'nf_zatca_integration.ubl_21_AddressType_zatca',
            'PartyType_template': 'nf_zatca_integration.ubl_21_PartyType_zatca',
            'TaxTotalType_template': 'nf_zatca_integration.ubl_21_TaxTotalType_zatca',
            'PaymentMeansType_template': 'nf_zatca_integration.ubl_21_PaymentMeansType_zatca',
        })

        vals['vals'].update({
            'profile_id': 'reporting:1.0',
            'document_type_code_attrs': {'name': self._nf_zatca_get_invoice_transaction_code(invoice)},
            'document_type_code': self._nf_zatca_get_invoice_type(invoice),
            ...
            'previous_invoice_hash': self._nf_zatca_get_previous_invoice_hash(invoice),
            'billing_reference_vals': self._nf_zatca_get_billing_reference_vals(invoice),
            'tax_total_vals': self._nf_zatca_get_additional_tax_total_vals(invoice, vals),
            # Due date is not required for ZATCA UBL 2.1
            'due_date': None,
        })

        vals['vals']['monetary_total_vals'].update(self._nf_zatca_get_monetary_vals(invoice, vals))
        self._nf_zatca_postprocess_line_vals(vals)
        return vals

    def _nf_zatca_postprocess_line_vals(self, vals):
        """
    ...
    def _nf_zatca_get_additional_tax_total_vals(self, invoice, vals):
        """
    ...
    def _nf_zatca_get_line_prepayment_vals(self, line, taxes_vals):
        """
    ...
    def _get_invoice_line_vals(self, line, line_id, taxes_vals):
        ...
        if not line.move_id._is_downpayment() and line._get_downpayment_lines():
            ...
            line_taxes = line.move_id._prepare_invoice_aggregated_taxes(
                filter_tax_values_to_apply=lambda l, t: not t["tax"].nf_zatca_is_retention,
        ...
        if not line.move_id._is_downpayment() and line._get_downpayment_lines():
            ...
            line_vals['prepayment_vals'] = self._nf_zatca_get_line_prepayment_vals(line, taxes_vals)
        else:
    ...
    def _get_tax_unece_codes(self, customer, supplier, tax):
        ...
        if supplier.country_id.code == 'SA':
            if not tax or tax.amount == 0:
                exemption_codes = dict(tax._fields["nf_zatca_exemption_reason_code"]._description_selection(self.env))
                if tax.nf_zatca_exemption_reason_code in TAX_EXEMPTION_CODES:
                    return _exemption_reason('E', tax.nf_zatca_exemption_reason_code)
                elif tax.nf_zatca_exemption_reason_code in TAX_ZERO_RATE_CODES:
                    return _exemption_reason('Z', tax.nf_zatca_exemption_reason_code)
                else:
                    return _exemption_reason('O', tax.nf_zatca_exemption_reason_code)
            else:
                return {
                    'tax_category_code': 'S',
