...
class AccountJournal(models.Model):
    ...
    nf_zatca_csr = fields.Binary(attachment=True, copy=False, groups="base.group_system",
                                help="The Certificate Signing Request that is submitted to the Compliance API")
    nf_zatca_csr_errors = fields.Html("Onboarding Errors", copy=False)

    nf_zatca_compliance_csid_json = fields.Char("CCSID JSON", copy=False, groups="base.group_system",
                                               help="Compliance CSID data received from the Compliance CSID API "
                                                    "in dumped json format")
    nf_zatca_production_csid_certificate_id = fields.Many2one(string="PCSID Certificate", comodel_name="certificate.certificate",
                                                          domain=[('is_valid', '=', True)])
    nf_zatca_production_csid_json = fields.Char("PCSID JSON", copy=False, groups="base.group_system",
                                               help="Production CSID data received from the Production CSID API "
                                                    "in dumped json format")
    nf_zatca_production_csid_validity = fields.Datetime(related="nf_zatca_production_csid_certificate_id.date_end")
    nf_zatca_compliance_csid_certificate_id = fields.Many2one(string="CCSID certificate", comodel_name="certificate.certificate",
                                                          domain=[('is_valid', '=', True)])
    nf_zatca_compliance_checks_passed = fields.Boolean("Compliance Checks Done", default=False, copy=False,
                                                      help="Specifies if the Compliance Checks have been completed successfully")

    nf_zatca_chain_sequence_id = fields.Many2one('ir.sequence', string='ZATCA account.move chain sequence',
                                                readonly=True, copy=False)

    nf_zatca_serial_number = fields.Char("Serial Number", copy=False,
                                        help="Unique Serial Number automatically filled when the journal is onboarded")

    nf_zatca_latest_submission_hash = fields.Char("Latest Submission Hash", copy=False,
                                                 help="Hash of the latest submitted invoice to be used as the Previous Invoice Hash (KSA-13)")

    def _nf_zatca_ready_to_submit_einvoices(self):
        """
            Helper function to know if the required CSIDs have been obtained, and the compliance checks have been
            completed
        """
        self.ensure_one()
        return self.sudo().nf_zatca_production_csid_json

    def _nf_zatca_api_onboard_sanity_checks(self):
        """
        ...
        move_ids = self.env['account.move'].search(
            [
                ('journal_id', '=', self.id),
                ('nf_zatca_chain_index', '!=', 0)
            ]
        )
        stuck_moves = [move for move in move_ids if not move._nf_zatca_is_in_chain()]
        if stuck_moves:
            raise UserError(_("Oops! The journal is stuck. Please submit the pending invoices to ZATCA and try again."))

    def _nf_zatca_set_csr_fields(self):
        '''
            Sets default values for CSR generation fields in Odoo, if their values do not exist
        '''
        self.ensure_one()
        # Avoid unnecessary write calls
        if self.nf_zatca_serial_number != str(self.id):
            self.nf_zatca_serial_number = self.id

    # ====== CSR Generation =======

    def _nf_zatca_csr_required_fields(self):
        """ Return the list of fields required to generate a valid CSR as per ZATCA requirements """
        return ['nf_zatca_private_key_id', 'vat', 'name', 'city', 'country_id', 'state_id']

    def _nf_zatca_generate_csr(self):
        """
            Generate a CSR for the Journal to be used for the Onboarding process and Invoice submissions
        """
        self.ensure_one()
        if any(not self.company_id[f] for f in self._nf_zatca_csr_required_fields()):
            raise UserError(
                _(
                    ...
                    fields="".join(
                        ...
                        for f in self._nf_zatca_csr_required_fields()
                        if not self.company_id[f]
                    ),
                ),
            )
        self._nf_zatca_reset_certificates()
        self.nf_zatca_csr = self.env['certificate.certificate'].sudo()._nf_zatca_get_csr_str(self)

    def _nf_zatca_reset_certificates(self):
        """
            Reset all certificate values, including CSR and compliance checks
        """
        for journal in self.sudo():
            journal.nf_zatca_csr = False
            journal.nf_zatca_production_csid_json = False
            journal.nf_zatca_compliance_csid_json = False
            journal.nf_zatca_compliance_checks_passed = False

    def _nf_zatca_api_onboard_journal(self, otp):
        """
        ...
        self._nf_zatca_api_onboard_sanity_checks()
        self._nf_zatca_set_csr_fields()
        ...
        try:
            ...
            ec_private_key_sudo = self.company_id.sudo().nf_zatca_private_key_id
            ...
            self._nf_zatca_reset_certificates()
            self.nf_zatca_csr_errors = e.args[0] or _("Journal could not be onboarded")

    def _nf_zatca_get_compliance_CSID(self, otp):
        """
            Request a Compliance Cryptographic Stamp Identifier (CCSID) from ZATCA
        """
        CCSID_data = self._nf_zatca_api_get_compliance_CSID(otp)
        ...
        cert_id = self.env['certificate.certificate'].sudo().create({
            ...
            'private_key_id': self.company_id.sudo().nf_zatca_private_key_id.id,
        }).id
        self.sudo().write({
            'nf_zatca_compliance_csid_json': json.dumps(CCSID_data),
            'nf_zatca_compliance_csid_certificate_id': cert_id,
            'nf_zatca_production_csid_json': False,
            'nf_zatca_compliance_checks_passed': False,
        })

    def _nf_zatca_get_production_CSID(self, OTP=None):
        """
        ...
        if not self_sudo.nf_zatca_compliance_csid_json or not self_sudo.nf_zatca_compliance_csid_certificate_id:
            raise UserError(_("Cannot request a Production CSID before requesting a CCSID first"))
        elif not self_sudo.nf_zatca_compliance_checks_passed:
            raise UserError(_("Cannot request a Production CSID before completing the Compliance Checks"))

        renew = False
        zatca_format = self.env.ref('nf_zatca_integration.zatca_account_edi_format')

        if self_sudo.nf_zatca_production_csid_json:
            time_now = zatca_format._nf_zatca_get_zatca_datetime(datetime.now())
            if zatca_format._nf_zatca_get_zatca_datetime(self_sudo.nf_zatca_production_csid_validity) < time_now:
                renew = True
            else:
                raise UserError(_("The Production CSID is still valid. You can only renew it once it has expired."))

        CCSID_data = json.loads(self_sudo.nf_zatca_compliance_csid_json)
        PCSID_data = self_sudo._nf_zatca_request_production_csid(CCSID_data, renew, OTP)
        if PCSID_data.get('error'):
            raise UserError(_("Could not obtain Production CSID: %s", PCSID_data['error']))
        self_sudo.nf_zatca_production_csid_json = json.dumps(PCSID_data)
        ...
        self.nf_zatca_production_csid_certificate_id = pcsid_certificate

    def _nf_zatca_get_compliance_files(self):
        """
    ...
    def _nf_zatca_run_compliance_checks(self):
        ...
        CCSID_data = json.loads(self_sudo.nf_zatca_compliance_csid_json)
        compliance_files = self._nf_zatca_get_compliance_files()
        for fname, fval in compliance_files.items():
            invoice_hash_hex = self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_generate_invoice_xml_hash(
                fval).decode()
            digital_signature = self.env.ref('nf_zatca_integration.zatca_account_edi_format')._nf_zatca_get_digital_signature(self.company_id, invoice_hash_hex).decode()
            prepared_xml = self._nf_zatca_prepare_compliance_xml(fname, fval, self_sudo.nf_zatca_compliance_csid_certificate_id, digital_signature)
            result = self._nf_zatca_api_compliance_checks(prepared_xml.decode(), CCSID_data)
            if result.get('error'):
            ...
            if result['validationResults']['status'] == 'WARNING':
                ...
                self.nf_zatca_csr_errors = Markup("<br/><br/><ul class='pl-3'><b>%s</b>%s</ul>") % (_("Warnings:"), warnings)
        ...
        self.nf_zatca_compliance_checks_passed = True

    def _nf_zatca_prepare_compliance_xml(self, xml_name, xml_raw, certificate, signature):
        """
            Prepare XML content to be used for Compliance checks
        """
        xml_content = self._nf_zatca_prepare_invoice_xml(xml_raw)
        signed_xml = self.env.ref('nf_zatca_integration.zatca_account_edi_format')._nf_zatca_sign_xml(xml_content, certificate, signature)
        if xml_name.startswith('simplified'):
            qr_code_str = self.env['account.move']._nf_zatca_get_qr_code(self, signed_xml, certificate, signature, True)
    ...
    def _nf_zatca_prepare_invoice_xml(self, xml_content):
        """
            Prepare the XML content of the test invoices before running the compliance checks
        """
        ubl_extensions = etree.fromstring(self.env['ir.qweb']._render('nf_zatca_integration.export_sa_zatca_ubl_extensions'))
        root = etree.fromstring(xml_content.encode())
        root.insert(0, ubl_extensions)
        ns_map = self.env['account.edi.xml.ubl_21.zatca']._nf_zatca_get_namespaces()
    ...
    def _nf_zatca_icv_onboarding(self):
        """
            Onboarding method to create or reset ICV sequence for the journal
        """
        self.ensure_one()
        if self.nf_zatca_chain_sequence_id:
            self.nf_zatca_chain_sequence_id.number_next = 1
            message = _("Journal re-onboarded with ZATCA successfully")
        else:
            self.nf_zatca_chain_sequence_id = self._nf_zatca_create_new_chain()
            message = _("Journal onboarded with ZATCA successfully")
        self.message_post(body=message)

    def _nf_zatca_create_new_chain(self):
        self.ensure_one()
        return self.env['ir.sequence'].create({
            'name': f'ZATCA account move sequence for Journal {self.name} (id: {self.id})',
            'code': f'nf_zatca_integration.account.move.{self.id}',
            'implementation': 'no_gap',
            'company_id': self.company_id.id,
        })

    def _nf_zatca_get_next_chain_index(self):
        self.ensure_one()
        if not self.nf_zatca_chain_sequence_id:
            self.nf_zatca_chain_sequence_id = self._nf_zatca_create_new_chain()
        return self.nf_zatca_chain_sequence_id.next_by_id()

    def _nf_zatca_get_last_posted_invoice(self):
        """
        ...
        return self.env['account.move'].search(
            ...
            limit=1, order='nf_zatca_chain_index desc'
        )

    # API Calls to ZATCA

    def _nf_zatca_api_get_compliance_CSID(self, otp):
        """
        ...
        if not self.nf_zatca_csr:
            raise UserError(_("Please, generate a CSR before requesting a CCSID"))
        request_data = {
            'body': json.dumps({'csr': self.nf_zatca_csr.decode()}),
            'header': {'OTP': otp}
        }
        return self._nf_zatca_call_api(request_data, ZATCA_API_URLS['apis']['ccsid'], 'POST')

    def _nf_zatca_api_get_production_CSID(self, CCSID_data):
        """
        ...
        request_data = {
            ...
            'header': {'Authorization': self._nf_zatca_authorization_header(CCSID_data)}
        }
        return self._nf_zatca_call_api(request_data, ZATCA_API_URLS['apis']['pcsid'], 'POST')

    def _nf_zatca_api_renew_production_CSID(self, PCSID_data, OTP):
        """
        ...
        if self.company_id.nf_zatca_api_mode == 'sandbox':
            auth_data = SANDBOX_AUTH
        request_data = {
            'body': json.dumps({'csr': self.nf_zatca_csr.decode()}),
            'header': {
                'OTP': OTP,
                'Authorization': self._nf_zatca_authorization_header(auth_data)
            }
        }
        return self._nf_zatca_call_api(request_data, ZATCA_API_URLS['apis']['pcsid'], 'PATCH')

    def _nf_zatca_api_compliance_checks(self, xml_content, CCSID_data):
        """
        ...
        request_data = {
            ...
            'header': {
                'Authorization': self._nf_zatca_authorization_header(CCSID_data),
                'Clearance-Status': '1'
            }
        }
        return self._nf_zatca_call_api(request_data, ZATCA_API_URLS['apis']['compliance'], 'POST')

    def _nf_zatca_get_api_clearance_url(self, invoice):
        """
            Return the API to be used for clearance. To be overridden to account for other cases, such as reporting.
        """
        return ZATCA_API_URLS['apis']['reporting' if invoice._nf_zatca_is_simplified() else 'clearance']

    def _nf_zatca_api_clearance(self, invoice, xml_content, PCSID_data):
        """
        ...
        request_data = {
            'body': json.dumps({
                "invoiceHash": invoice_hash,
                "uuid": invoice.nf_zatca_uuid,
                "invoice": b64encode(xml_content.encode()).decode()
            }),
            'header': {
                'Authorization': self._nf_zatca_authorization_header(PCSID_data),
                'Clearance-Status': '1'
            }
        }
        url_string = self._nf_zatca_get_api_clearance_url(invoice)
        return self._nf_zatca_call_api(request_data, url_string, 'POST')

    # ====== Certificate Methods =======

    def _nf_zatca_request_production_csid(self, csid_data, renew=False, otp=None):
        """
            Generate company Production CSID data
        """
        self.ensure_one()
        return (
            self._nf_zatca_api_renew_production_CSID(csid_data, otp)
            if renew
            else self._nf_zatca_api_get_production_CSID(csid_data)
        )

    def _nf_zatca_api_get_pcsid(self):
        """
            Get CSIDs required to perform ZATCA api calls, and regenerate them if they need to be regenerated.
        """
        self.ensure_one()
        self_sudo = self.sudo()
        if not self_sudo.nf_zatca_production_csid_json or not self_sudo.nf_zatca_production_csid_certificate_id:
            raise UserError(_("Please, make a request to obtain the Compliance CSID and Production CSID before sending "
                            "documents to ZATCA"))
        certificate = self_sudo.nf_zatca_production_csid_certificate_id
        if not certificate.is_valid and self.company_id.nf_zatca_api_mode != 'sandbox':
            raise UserError(_("Production certificate has expired, please renew the PCSID before proceeding"))
        return json.loads(self_sudo.nf_zatca_production_csid_json), certificate.id

    # ====== API Helper Methods =======

    def _nf_zatca_call_api(self, request_data, request_url, method):
        """
            Helper function to make api calls to the ZATCA API Endpoint
        """
        api_url = ZATCA_API_URLS[self.company_id.nf_zatca_api_mode]
        request_url = urljoin(api_url, request_url)
        status_code = False
        try:
            request_response = requests.request(method, request_url, data=request_data.get('body'),
                                                headers={
                                                    **self._nf_zatca_api_headers(),
    ...
    def _nf_zatca_api_headers(self):
        """
    ...
    def _nf_zatca_authorization_header(self, CSID_data):
        """
    ...
    def _nf_zatca_load_edi_demo_data(self):
        self.ensure_one()
        self.company_id.nf_zatca_private_key_id = self.env['certificate.key']._generate_ec_private_key(self.company_id)
        self.write({
            'nf_zatca_serial_number': 'SIDI3-CBMPR-L2D8X-KM0KN-X4ISJ',
            'nf_zatca_compliance_checks_passed': True,
            'nf_zatca_csr': b'',
            'nf_zatca_compliance_csid_json': """{"requestID": 1234567890123, "dispositionMessage": "ISSUED", "binarySecurityToken": "", "secret": "uMpSz85cV0h/e/uqpJ+FaZkdYZ76uoaRYOevGufcup0=", "errors": null}""",
            'nf_zatca_production_csid_json': """{"requestID": 30368, "tokenType": "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-x509-token-profile-1.0#X509v3", "dispositionMessage": "ISSUED", "binarySecurityToken": "", "secret": "CkYsEXfV8c1gFHAtFWoZv73pGMvh/Qyo4LzKM2h/8Hg="}"""
        })
