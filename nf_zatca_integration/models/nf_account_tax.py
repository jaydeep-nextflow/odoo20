...
class AccountTax(models.Model):
    ...
    nf_zatca_is_retention = fields.Boolean("Is Retention", default=False,
                                          help="Determines whether or not a tax counts as a Withholding Tax")

    nf_zatca_exemption_reason_code = fields.Selection(string="Exemption Reason Code",
                                                     selection=EXEMPTION_REASON_CODES, help="Tax Exemption Reason Code (ZATCA)")

    @api.onchange('amount')
    def onchange_amount(self):
        super().onchange_amount()
        self.nf_zatca_is_retention = False

    @api.constrains("nf_zatca_is_retention", "amount", "type_tax_use")
    def _nf_zatca_constrain_is_retention(self):
        for tax in self:
            if tax.amount >= 0 and tax.nf_zatca_is_retention and tax.type_tax_use == 'sale':
